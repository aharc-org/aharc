...
if [ ${#REP_FILES[@]} -eq 0 ]; then
    # No .rep file → use BASENAME
    REPNAME="$BASENAME"

elif [ ${#REP_FILES[@]} -eq 1 ]; then
    # One .rep file → extract name
    REP_FILE="${REP_FILES[0]}"
    REPNAME=$(basename "$REP_FILE" .rep)

else
    # More than one .rep file → error
    echo "❌ Expected none or one .rep file, found ${#REP_FILES[@]}"
    exit 1
fi

# Build expected URL
EXPECTED_URL="git@github.com:${BASENAME}-org/${REPNAME}.git"

echo "Using repo: $EXPECTED_URL"
